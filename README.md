...
This type is different in it's base, because the action decision, doesn't come from the neural network, but from an algorithm, that detects the object(face) and moves to the direction of the object. I've made it in different variations:
  - 1. Looking for a face in the frame from car and moving to the direction of the face movement
  - 2. Choosing a specific object and moving to the direction of the object movement
  - 3. The following was made with webcam movement only or with the movement of the whole car, suing wheels motors rotation 
  ![](https://supercoolserver.azurewebsites.net/assets/img/ml_car_face_following.gif)
